...
public:
    ...
    pair<int, int> get(int number) {
        ...
        return make_pair(-1, -1); // Return a default pair if number is not found
    }


    pair<int, int>* RankGet(int r) {
        r--;
        TreeNode<T>* currentNode = root;
        while (currentNode) {
        ...
        return nullptr; // Return nullptr if rth smallest pair is not found
    }
};
...
int main(void) {
    ...
    //10.
    pair<int, int>* rank = bst.RankGet(5);
        cout << "번호: " << rank->first << " ";
        cout << "점수: " << rank->second;
}
